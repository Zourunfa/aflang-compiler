...
/**
 * Vec<impl Fn(String) -> ParseResult> 是一个 Rust 中的类型标记，
 * 表示一个存储了实现了 Fn(String) -> ParseResult trait
 *  的闭包的动态数组（即 Vec）。这里的 Fn(String) -> ParseResult
 * 表示一个接受
 *  String 类型参数并返回 ParseResult 类型的闭包。
 *
 */
fn any_of<'a>(parsers: Vec<impl Fn(String) -> ParseResult>) -> impl Fn(String) -> ParseResult {
    return move |input: String| {
        ...
        return Err(ParseErr::new("any of_err"));
    };
}
...
fn one_or_more(parser: impl Fn(String) -> ParseResult) -> impl Fn(String) -> ParseResult {
    return move |mut input: String| {
        ...
        match parser(input.clone()) {
            ...
            Err(err) => return Err(ParseErr::wrap("one_or_more err", err)),
        }
...
fn whitesapce<'a>() -> impl Fn(String) -> ParseResult {
...
fn unit(input:String) ->ParseResult{
...
fn int(input:String) -> ParseResult {
  let sign = zero_or_more(parse_char('-'));
  match sign(input){
      Ok((input,_))=>{
        match unit(input) {
